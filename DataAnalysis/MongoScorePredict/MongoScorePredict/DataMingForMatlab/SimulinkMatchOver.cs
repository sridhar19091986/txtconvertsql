    ...
    public class SimulinkMatchOver : IDisposable
    {
        ...
        public void CreateLiveCollection()
        {
            ...
            foreach (var today_now in cals)
            {
                ...
                SimulinkMatchOverDocument smnd = new SimulinkMatchOverDocument();
                ...
                smnd.full_host_goals = (int)today_now.result_score.full_host_goals;
                smnd.full_away_goals = (int)today_now.result_score.full_away_goals;
                smnd.full_310value = ConverResult310(today_now.result_score.full_310value);

                mongocrud.MongoDropColCreateCol.Insert(smnd);
            }
        }

        public int ConverResult310(int? value310)
        {
            switch (value310)
            {
                case 3:
